...
public class EllipsoidRegionViewModelTests
{
    ...
    {
        var viewModel = new EllipsoidRegionViewModel(new EllipsoidTissueRegion(), "TestEllipsoidTissue");
        Assert.That(viewModel.Name, Is.EqualTo("TestEllipsoidTissue" + StringLookup.GetLocalizedString("Label_Tissue")));
        Assert.That(viewModel.IsEllipsoid, Is.True);
        Assert.That(viewModel.IsLayer, Is.False);
        Assert.That(viewModel.Units, Is.EqualTo(StringLookup.GetLocalizedString("Measurement_mm")));
        Assert.That(viewModel.OpticalPropertyVM != null, Is.True);
    ...
    {
        var ellipsoidTissueRegion = new EllipsoidTissueRegion {RegionOP = {G = 0.8, Mua = 0D, Mus = 1E-10}};
        ellipsoidTissueRegion.RegionOP.G = 1.4;
        var viewModel = new EllipsoidRegionViewModel(ellipsoidTissueRegion, "TestEllipsoidTissue");
        Assert.That(ellipsoidTissueRegion.IsAir(), Is.True);
        Assert.That(viewModel.Name, Is.EqualTo("TestEllipsoidTissue" + StringLookup.GetLocalizedString("Label_Air")));
    }
